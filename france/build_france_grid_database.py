...
def enrich_rte_with_osm(rte_substations: List[Substation], osm_substations: List[Dict]) -> List[Substation]:
    """Enrich RTE substations with OSM coordinates using indexed lookup."""
    print("\n" + "="*60)
    ...
    print(f"  RTE substations matched to OSM: {matched}/{len(rte_substations)} ({100*matched/len(rte_substations):.1f}%)")
